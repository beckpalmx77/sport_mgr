...
if(!empty($_POST["classid"])) 
{
 ...
 $stmt = $dbh->prepare("SELECT FirstName,StudentId FROM tblstudents WHERE ClassId= :id order by FirstName");
 $stmt->execute(array(':id' => $cid));
 ?><option value="">Select Category </option><?php
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
  ?>
  <option value="<?php echo htmlentities($row['StudentId']); ?>"><?php echo htmlentities($row['FirstName']); ?></option>
  <?php
 }
}
